...
BRANDS = {
    "superbetin": {
        ...
        # GÜNCELLENDİ 11 Ağu 2026 — superbetin2087.com artık pasif, aktif adres superbetin2088.com
        "active_domains": ["superbetin.com", "superbetin2088.com"],
    ...
    "betsat": {
        ...
        # GÜNCELLENDİ 11 Ağu 2026 — betsat1610.com artık pasif, aktif adres betsat1611.com
        "active_domains": ["betsat.com", "betsat1611.com"],
...
HOSTS = {
    ...
    "digitalocean":    {"name": "DigitalOcean LLC",                     "abuse": ["abuse@digitalocean.com"]},  # YENİ 31 Tem — superbetin-tr.top üzerinden ilk kez görüldü. NOT: DigitalOcean email'i X-ARF formatı istiyor, otomatik email genelde bounce eder — web form yedeği: https://www.digitalocean.com/company/contact/#abuse
    ...
    "frostyhosting":   {"name": "FrostyHosting (Belenkii Ivan Alexandrovich, RU)", "abuse": ["frostyhosting@proton.me"]},   # YENİ 10 Ağu 2026 — betsatdirect.co üzerinden ilk kez görüldü (elinore/patrick ambiguous cluster'ında zaten vardı, artık standalone da eklendi)
}
# ============================================================
# WEB-FORM-ONLY REGISTRAR/HOST'LAR — bunlar email KABUL ETMİYOR.
# Script bu registrar'ları tespit ederse ASLA otomatik email göndermeye
# çalışmaz; sadece bilgilendirme yapıp elle form doldurulması gerektiğini
# işaretler (çağıran koddaki registrar_info["form_required"] kontrolüne bak).
# 10-11 Ağu 2026 tarihli "Abuse Report Kanalları Rehberi" referans alınmıştır.
# ============================================================
...
def web_form_only_match(registrar_name):
    """Registrar adı web-form-only listesindeki anahtar kelimelerden
    birini içeriyorsa ilgili kaydı döndürür, yoksa None."""
...
CLUSTER_MAP = {
    ...
    frozenset({"desi", "josh"}):        "swissnet",       # YENİ 10 Ağu 2026 — superbetin2088.co (SwissNet LLC, abuse@swissnetwork.io)
    frozenset({"brodie", "laylah"}):    "vpsdedicated_flashwisp",   # YENİ 10 Ağu 2026 — betsat-agustos-girisi.co (VPS Dedicated LLC, abuse@flashwisp.com.ng)
    frozenset({"abdullah", "liv"}):     "swissnet",       # YENİ 11 Ağu 2026 — betin-onlineadresi.icu (SwissNet LLC, abuse@swissnetwork.io)
}
...
# ============================================================
# DAHA ÖNCE RAPORLANMIŞ DOMAINLERİ YÜKLE (11 Ağu 2026 — YENİ)
# ============================================================
# Bu oturumda Trustname'e aynı domain için 2-4 kez ayrı ticket açıldığı
# (örn. superbetin2151.com #ABS-48012/48014/48021/48155, betsat-betsatorjinal.com
# #ABS-47909/48004, mbetsat1610.com #ABS-47910/48016/48017) tekrar tekrar
# görüldü. Panel bunu SESSİZCE engellemiyor (bazen kasıtlı bir tekrar —
# örn. yeni kanıtla RFI cevabı — meşru olabilir), ama en azından operatöre
# NET bir uyarı veriyor ki yanlışlıkla aynı raporu tekrar göndermesin.
def load_reported():
    reported = set()
...
async def main():
    ...
    # YENİ 11 Ağu 2026 — Bu run'ın DIŞINDA, ÖNCEKİ run'larda zaten
    # raporlanmış domainleri de kontrol et. Bu, Ağustos ayı boyunca
    # Trustname'de (superbetin2151.com'a 4, betsat-betsatorjinal.com'a
    # 2, mbetsat1610.com'a 3 ayrı ticket açılması gibi) tekrarlanan
    # yanlışlıkla-çifte-gönderim sorununu SESSİZCE engellemiyor (bazen
    # kasıtlı bir tekrar meşrudur — örn. RFI cevabı için yeni kanıtla
    # yeniden gönderim), ama operatöre açık bir uyarı veriyor.
